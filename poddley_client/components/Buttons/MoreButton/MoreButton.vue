<template>
    <HeadlessMenu as="div" class="absolute z-50 flex h-9 w-9 items-center justify-center text-left">
        ...
        <transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0"
            ...
            <HeadlessMenuItems
                class="bg-white ring-black absolute right-0 top-full z-50 mt-2 w-56 rounded-md shadow-lg ring-1 ring-opacity-5 focus:outline-none">
...
const loadEntireTranscript = async (episodeGuid: string) => {
    ...
    let segmentHits: SegmentHit[] = searchResults.value.hits[props.index].subHits as unknown as SegmentHit[];

    // Doing it:
    segmentHits = segmentHits.reduce((accumulatedSegments: SegmentHit[], segHit: SegmentHit) => {
