<template>
  <HeadlessMenu as="div" class="inset-y-0 left-0 flex h-10 w-10 origin-top-left items-center p-0 text-left" v-slot="{ open, close }">
    <HeadlessMenuButton
      ...
      <HeadlessMenuItems @click="close" class="bg-white ring-black absolute left-0 top-full z-50 m-0 w-56 rounded-md shadow-lg ring-1 ring-opacity-5 focus:outline-none">
        <div class="text-gray-700 bg-white w-full rounded-lg px-0 py-1">
          ...
          <HeadlessMenuItem class="group flex w-full flex-row flex-nowrap items-center no-underline">
            <NuxtLink exact to="/about" :class="`group flex justify-between gap-x-0 px-3 py-2 text-base hover:bg-gray-300`">
              About
