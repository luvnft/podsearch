<template>
    <div ref="listRef" class="relative">
        <button v-for="(subHit, index) in props.searchEntry.subHits" @click="goToAudioTime(subHit.start)"
            ...
            :class="`${(((subHit.start - 1 <= props.currentPlayingTime) && (subHit.end - 1 >= props.currentPlayingTime)) && (props.currentPlayingTime > 0.3)) ? 'highlight' : props.currentPlayingTime < 0.1 ? 'toggleDeepStyling' : ''}`"
            ...
            if (listRef.value && props.searchEntry.subHits) {
                Array.from(listRef.value.children).forEach((item: any, index) => {
                    if (props.searchEntry.subHits) {
                        ...
                        if (subHit.start - 1 <= props.currentPlayingTime && subHit.end - 1 >= props.currentPlayingTime) {
                            scrollIntoView(item, {
                                behavior: 'smooth',
