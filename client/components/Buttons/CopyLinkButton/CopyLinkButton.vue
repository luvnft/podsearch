<template>
    <div class="text-gray-600 flex h-full w-full flex-col justify-center gap-1 no-underline">
        <button @click="handleCopyClick" type="button" title="copyLinkButton"
            ...
            <svg-icon name="copylink"
                class="h-5 w-5 fill-gray-400 p-0.5 group-hover:fill-gray-500 dark:group-hover:fill-gray-300"
                aria-hidden="true" />
        </button>
    </div>
</template>
...
async function handleCopyClick() {
    navigator.clipboard.writeText(process.env.HOMEPAGE_URL + "/quote?id=" + props.segmentId);
    showText();
}
