    ...
    <HeadlessMenu as="div" class="inset-y-0 left-0 flex h-10 w-10 origin-top-left items-center p-0 text-left"
        ...
        <transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0"
            ...
            <HeadlessMenuItems @click="close"
                class="bg-white ring-black absolute left-0 top-full z-50 m-0 w-56 rounded-md shadow-lg ring-1 ring-opacity-5 focus:outline-none">
                <div class="text-gray-700 bg-white w-full rounded-lg px-0 py-1">
                    <HeadlessMenuItem
                        class="group flex w-full flex-row flex-nowrap items-center no-underline hover:bg-gray-300">
                        <NuxtLink exact to="/" :class="`flex justify-between gap-x-0 px-3 py-2 text-base`">
                            Home
                            <svg-icon name="home"
                                class="h-4 w-4 fill-gray-300 p-0 group-hover:fill-gray-700 dark:fill-gray-600 dark:group-hover:fill-gray-300" />
                        </NuxtLink>
                    </HeadlessMenuItem>
                    <HeadlessMenuItem class="group flex w-full flex-row flex-nowrap items-center no-underline">
                        <NuxtLink exact to="/about"
                            :class="`group flex justify-between gap-x-0 px-3 py-2 text-base hover:bg-gray-300`">
                            About
                            <svg-icon name="info"
                                class="h-4 w-4 fill-gray-300 p-0 group-hover:fill-gray-700 dark:fill-gray-600 dark:group-hover:fill-gray-300" />
                        </NuxtLink>
